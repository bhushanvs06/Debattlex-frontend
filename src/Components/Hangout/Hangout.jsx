...
// Unified animation hook: one rAF loop on mobile, three separate hooks on desktop
function useAnimationValues() {
  const mobile = isMobile();

  // Mobile path: single rAF loop computing all three values in one callback
  const [mobileValues, setMobileValues] = useState({ rotate: 0, breath: 1, float: 0 });
  const mobileRaf = useRef(null);
  const mobilePrev = useRef(null);

  useEffect(() => {
    if (!mobile) return;
    const tick = (ts) => {
      if (mobilePrev.current !== null) {
        const dt = ts - mobilePrev.current;
        setMobileValues(prev => ({
          rotate: (prev.rotate + dt * 0.06) % 360,
          breath: 1 + Math.sin(ts / 800) * 0.04,
          float: Math.sin(ts / 900) * 8,
        }));
      }
      mobilePrev.current = ts;
  ...
  // Desktop path: delegate to the three individual hooks (unchanged)
  const desktopRotate = useRotate(1);
  const desktopBreath = useBreath();
  const desktopFloat = useFloat();

  if (mobile) {
    return { rotate: mobileValues.rotate, breath: mobileValues.breath, float: mobileValues.float };
  }
  return { rotate: desktopRotate, breath: desktopBreath, float: desktopFloat };
}
...
function BounceD({ color, intervalMs = 50 }) {
  const [sc, setSc] = useState(0.35);
  useEffect(() => {
    const id = setInterval(() => setSc(0.35 + Math.abs(Math.sin(Date.now() / 300)) * 0.65), intervalMs);
...
export default function Hangout() {
  ...
  const { rotate: orbDeg, breath, float: floatY } = useAnimationValues();
        ...
        {isMatching && (
          <div style={{ textAlign: "center", color: "white", position: "relative", padding: "clamp(28px,5vw,48px) clamp(24px,5vw,56px)", ...fadeUp, maxWidth: 560, width: "92%" }}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "center", margin: "18px 0 34px" }}>
              {[C.violet, C.pink, C.fuchsia].map((color, i) => <BounceD key={i} color={color} intervalMs={isMobile() ? 200 : 50} />)}
            </div>
